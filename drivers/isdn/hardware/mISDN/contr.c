...
static int misdn_proc_show(struct seq_file *m, void *v);

static int misdn_proc_open(struct inode *inode, struct file *file) {
	return single_open(file, misdn_proc_show, PDE(inode)->data);
}

const struct file_operations misdn_proc_fops = {
	.owner		= THIS_MODULE,
